...
def jr_east_insert(db_connection):
    ...
    try:
        with open(JR_EAST_FILE_NAME, 'r', encoding="utf_8") as jr_east_csv:
            reader = csv.reader(jr_east_csv)
            # ヘッダは使わないので読み飛ばし
            header = next(reader)
...
def tokyo_metro_insert(db_connection):
    ...
    try:
        with open(TOKYO_METRO_FILE_NAME, 'r', encoding="utf_8") as tokyo_metro_csv:
            reader = csv.reader(tokyo_metro_csv)
            # ヘッダは使わないので読み飛ばし
            header = next(reader)
